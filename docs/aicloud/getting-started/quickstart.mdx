...
- 关于 GPU
  ...
  - 如果没有 Nvidia GPU，部署后的环境就无法运行 ollama 和 vllm 这种推理容器实例，但可以运行 OpenClaw 和 dify 这种不依赖 GPU 的应用容器实例。
...
| HermesAgent | AI应用 | Nous Research 开源自主 Agent | 不需要 | [HermesAgent 快速开始](../guides/llm-app/hermes#quickstart) |
